...
//!
//! State is passed via FunctionTemplate data (External containing Rc<T>).
//! Use `{fn_name}_v8_template(scope, &state)` to register the function.
//!
//! ```ignore
...
/// ```
///
/// With state (passed via FunctionTemplate data):
/// ```ignore
/// #[glue_v8::method(state = Rc<TimerState>)]
/// fn schedule_timeout(scope: &mut v8::PinScope, state: &Rc<TimerState>, id: u64, delay: u64) {
///     let _ = state.scheduler_tx.send(SchedulerMessage::ScheduleTimeout(id, delay));
/// }
///
/// // Registration:
/// let state = Rc::new(TimerState { ... });
/// let func = schedule_timeout_v8_template(scope, &state).get_function(scope).unwrap();
/// ```
///
/// With custom JS name:
/// ```ignore
...
pub fn method(attr: TokenStream, item: TokenStream) -> TokenStream {
        ...
        .filter_map(|arg| {
            if let FnArg::Typed(pat_type) = arg {
                if let Pat::Ident(pat_ident) = &*pat_type.pat {
                    ...
                    // Skip 'state' - will be extracted from FunctionTemplate data
                    if name_str == "state" {
                        has_state = true;
                        return None;
                    }
    ...
    // Generate template function name for stateful functions
    ...
    let expanded = if attrs.fast {
        ...
        // Non-fast with state: generate wrapper + template function
        ...
        // Standard mode: only slow path, no state
        quote! {
            #input_fn
